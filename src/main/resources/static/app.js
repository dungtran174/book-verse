...
function renderPagination(data) {
    ...
    let html = `<button class="nav-txt" ${data.first ? 'disabled' : ''} onclick="goToPage(${data.page - 1})">◀ Trước</button>`;
    for (let i = 0; i < data.totalPages; i++) {
        html += `<button class="${i === data.page ? 'active' : ''}" onclick="goToPage(${i})">${i + 1}</button>`;
    }
    html += `<button class="nav-txt" ${data.last ? 'disabled' : ''} onclick="goToPage(${data.page + 1})">Sau ▶</button>`;
    div.innerHTML = html;
}
